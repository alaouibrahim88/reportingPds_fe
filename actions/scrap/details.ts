...
export async function getZoneSubDetails(
  year: number,
  displayType: string = "Qty",
  month: string = "3",
  zonename: string
  ...
  try {
    ...
    const token = await getCookieValue("access_token");
    const url = new URL(`${process.env.NEXT_PUBLIC_API_ENDPOINT}${Endpoints.scrap.details.zoneDetailsContent}`);
    url.searchParams.append('annee', validYear.toString());
    url.searchParams.append('typeaffichage', validDisplayType.toString());
    url.searchParams.append('mois', validMonth.toString());
    url.searchParams.append('zone', zonename.toString());
