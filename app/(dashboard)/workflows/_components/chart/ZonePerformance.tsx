...
import { EfficiencyTracking } from "@/types";

interface ZonePerformanceProps {
  data: EfficiencyTracking[] | undefined;
  activeIndex: number;
        ...
        {data?.map((item, index) => (
          ...
          >
            <div className="flex items-center justify-between mb-1.5">
              <div>
                <p className="font-medium text-sm">{item.zone_nom}</p>
                <div className="flex items-center text-[10px] text-muted-foreground">
                  <span>{item.heures_reel} standard hours</span>
                  <span className="mx-1">•</span>
                  <span className=" text-red-500/70 dark:text-red-400/70 font-medium">
                    {item.heures_reel} reel Hours
                  </span>
              ...
              <div className="text-right">
                <p className="font-medium text-sm">{item.efficience}%</p>
